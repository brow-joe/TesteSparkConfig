...
public class Poisson{
	...
	public static void main( String[ ] args ) {
		...
		labelCounter.entrySet().forEach( entry -> {
			...
			entry.getValue().entrySet().forEach( target -> {
				...
				target.getValue().entrySet().forEach( outcome -> {
					double coeficient = calculePoisson( outcome.getValue(), outcomeDataCounter.get( outcome.getKey() ) );
		...
		outcomeDataCounter.entrySet().forEach( outcome -> {
			double coeficient = calculePoisson( outcome.getValue(), total );
			outcomeCoeficients.put( outcome.getKey(), coeficient );
		} );
	...
	private static double calculePoisson( long target, long outcome ) {
		double alpha = 0.01;
		double p1 = target + alpha;
		double p2 = outcome + alpha;
		double coeficient = ( Math.pow( p2, p1 ) * Math.pow( Math.E, p2 * -1 ) ) / BigIntegerMath.factorial( (int) p1 ).doubleValue();
		return coeficient;
	}
}
